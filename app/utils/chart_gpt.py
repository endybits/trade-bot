...
from app.utils.prompts import (
    few_shot_code_to_chart_template,
    few_shot_code_to_chart_template_alternative,
)
...
def gpt_generate_chart_code(base_prompt: str):
    response = openai.Completion.create(
        model="text-davinci-003", prompt=base_prompt, temperature=0.01, max_tokens=1000
    )
...
def make_chart(user_question: str, sql_query: str, db_data: str):
    ...
    try:
        ...
        if file_dict["filename"] and file_dict["path"]:
            print(file_dict)
            chart_url = uploadFileToS3(
                path=file_dict["path"], filename=file_dict["filename"]
            )
            return chart_url
